...
class DeltakerServiceTest {
    ...
    @Test
    fun `oppdaterDeltaker(deltakerOppdatering) - utkast avbrytes for ny deltakelse - setter kan endres paa den forrige avsluttede deltakelsen om den ikke har status AVBRUTT_UTKAST, FEILREGISTRERT eller SAMARBEIDET_MED_ARRANGOREN_ER_AVBRUTT`() {
        val gammelDeltaker = TestData.lagDeltaker(
            status = TestData.lagDeltakerStatus(type = DeltakerStatus.Type.HAR_SLUTTET),
        )
        TestRepository.insert(gammelDeltaker)
        ...
        service.oppdaterDeltaker(nyDeltakerOppdaterUtkast)

        service.get(gammelDeltaker.id).getOrThrow().kanEndres shouldBe false

        val deltakeroppdateringAvbruttUtkast = Deltakeroppdatering(
            id = nyDeltaker.id,
            startdato = null,
            sluttdato = null,
            dagerPerUke = null,
            ...
            bakgrunnsinformasjon = "Tekst",
            deltakelsesinnhold = null,
            status = TestData.lagDeltakerStatus(type = DeltakerStatus.Type.AVBRUTT_UTKAST),
            erManueltDeltMedArrangor = false,
            historikk = emptyList(),
        )

        service.oppdaterDeltaker(deltakeroppdateringAvbruttUtkast)

        val nyDeltakerFraDb = service.get(nyDeltaker.id).getOrThrow()
        nyDeltakerFraDb.status.type shouldBe DeltakerStatus.Type.AVBRUTT_UTKAST

        val deltakerFraDb = service.get(gammelDeltaker.id).getOrThrow()
        deltakerFraDb.kanEndres shouldBe true
    }

    @Test
    fun `oppdaterDeltaker(deltakerOppdatering) - utkast avbrytes for ny deltakelse - beholder kan ikke endres paa den forrige avsluttede deltakelsen om den har status AVBRUTT_UTKAST, FEILREGISTRERT eller SAMARBEIDET_MED_ARRANGOREN_ER_AVBRUTT`() {
        val gammelDeltaker = TestData.lagDeltaker(
            status = TestData.lagDeltakerStatus(type = DeltakerStatus.Type.AVBRUTT_UTKAST),
        )
        TestRepository.insert(gammelDeltaker)
